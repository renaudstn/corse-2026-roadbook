(() => {
  ...
  const dayKey = IS_ALT ? "corse2026-alt-selected-day" : "corse2026-selected-day";
  ...
  const paramDay = dayParam
    ? DATA.days.find((d) => d.id === dayParam || d.date === dayParam) || null
    : null;
  // Highlight / "today" ribbon: URL day (id or ISO) → that day's date; else real today
  const nowISO = paramDay?.date || todayISO();
  ...
  const openTip = (title, text, eyebrow = "Info utile") => {
    if (!tipLayer) return;
    const eye = $("#tipEyebrow");
    const titleEl = $("#tipTitle");
    const bodyEl = $("#tipBody");
  ...
  /* ---------- Overview (main page only — skip if elements absent) ---------- */
  const stats = [
  ...
  if (today && ribbon) {
    ...
    if (todayTitle) todayTitle.textContent = `${today.weekday} ${today.dayNum} · ${today.short || today.title}`;
  ...
  const updateSwitcher = () => {
    ...
    if (!day) return;
    const i = dayIndex(selectedId);
    const switchDate = $("#switchDate");
    const switchTitle = $("#switchTitle");
    if (switchDate) switchDate.textContent = `${day.weekday} ${day.dayNum} août`;
    if (switchTitle) switchTitle.textContent = day.short || day.title;
    const prevBtn = $("#prevDay");
    const nextBtn = $("#nextDay");
    if (prevBtn) prevBtn.disabled = i <= 0;
    if (nextBtn) nextBtn.disabled = i >= DATA.days.length - 1;
  };
  ...
  const renderDay = () => {
    ...
    stage.innerHTML = `
      <article class="day-card" id="day-${day.id}">
        ...
        ${day.enrich ? `<p class="enrich"><strong>Conseil terrain ·</strong> ${day.enrich}</p>` : ""}
        ...
        <div class="day-pager">
          <button type="button" class="day-pager__btn" data-go-day="${prev?.id || ""}" ${prev ? "" : "disabled"}>
            <span>← Précédent</span>
            <strong>${prev ? `${prev.dayNum} · ${prev.short || prev.title}` : "—"}</strong>
          </button>
          <button type="button" class="day-pager__btn day-pager__btn--next" data-go-day="${next?.id || ""}" ${next ? "" : "disabled"}>
            <span>Suivant →</span>
            <strong>${next ? `${next.dayNum} · ${next.short || next.title}` : "—"}</strong>
          </button>
        </div>
      </article>`;
  ...
  /* Hero wow: entrance + parallax + menu solidify off-hero (only if elements exist) */
  const hero = $("#accueil");
